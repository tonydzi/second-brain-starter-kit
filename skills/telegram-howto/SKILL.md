...
- **On-demand:** `PYTHONUTF8=1 python "$IMPORTS_ROOT/dialogs/refresh_chats.py" --account [рабочий аккаунт]` (or `[рабочий аккаунт]`); appends/updates one account's live dialogs into chats.db via its OWN dedicated read session, PID-locked. Trigger words: "обнови индекс чатов", "refresh chat index".
- **Nightly:** scheduled task `Telegram Chats Index Refresh` (daily 02:30) runs `refresh_chats_nightly.cmd` for 4 accounts ([рабочий аккаунт], [рабочий аккаунт], [рабочий аккаунт], TONYDZI — all with sessions in `dialogs/.env`) and appends to `_refresh.log`. Check the LIVE status there, never from memory — do not hard-code a dated "ran / didn't run" here, that state changes nightly.
...
- `search_contacts` → people / bots / [аккаунт] only. NOT private group titles.
...
- **Saved Messages** = `[id]` (Anton's own user id, @[рабочий аккаунт]). Use the NUMBER, not `"me"` (this MCP rejects "me"). Mixed clipboard: lead [аккаунт], links, forwards, auto-reports, FB-diary drafts. Voice here is rare and NOT auto-transcribed.
- **Telegram service** = `777000` — login codes / OTP land here. **Fetch the code YOURSELF**, don't ask Anton (you're already connected to the account): `get_history(777000, account=<acct>, limit=1)` → parse `Login code: NNNNN`. Codes are short-lived → fetch + use immediately. ⭐ **2FA: ALSO self-serve, don't ask Anton** (updated 2026-08-05 — the old "never store it" line here was stale prose that outlived the fix): the cloud password lives in `~/.claude/secrets/telegram_2fa.env` per account (`TELEGRAM_2FA_[рабочий аккаунт]` / `_[рабочий аккаунт]` / `_[рабочий аккаунт]` / `_TONYDZI` + `_DEFAULT`), and `tg_session_init.py` injects it as `TG_2FA_PASSWORD` without ever printing it. Canon: vault `reglament-kody-vhoda-i-otp-assistent-dostaet-sam-iz-sluzhebnogo-chata`, memory `telegram-otp-self-fetch`, `credential-store`. **Connecting/relogging a machine is not this skill's job → `/tg-connect`.**
- **Content hub** = `-[id]` — "00 Архив ГОЛОСА и ТЕКСТА … для любых моих постов КОНТЕНТ мой посты" (Supergroup). Anton's personal dictation dump for content. **Auto-transcribed** (see §2).
...
In `-[id]`, each Anton **voice** message gets two auto-replies from bot **"Personal Audio Summary"** (both `reply_to` the voice id):
...
- **⭐ Re-read the thread IMMEDIATELY before sending (2026-08-14).** Your earlier recall of the thread goes stale in minutes because **parallel sessions on this machine write to the same threads**. One `get_history(chat_id, limit=3-5)` right before send — 1 call, 0 tokens. Measured: a session read the thread clean at 09:08, another session messaged the same person at 09:12, he replied «Заебывет» at 09:13, and the first session sent a friendly gift at 09:43 — 4 messages from 3 sessions in 5.5h to someone who had asked for quiet. Also: **register the touch in `_outreach/outreach_log.py` right after sending** — the 09:12 session didn't, so the next session's `precheck` was blind to it. Canon: `reglament-pered-otpravkoy-perechitay-chto-realno-v-pole` §Расширение 14.08.

- **Tool call hangs >300s ≠ dead daemon (2026-08-14).** A dialogs-scanning call (`get_direct_chat_by_contact` / `search_dialogs` / `get_chats`) on the shared SSE daemon can exceed the MCP idle timeout purely from **accumulated FloodWait** («Sleeping 21-25s on GetDialogsRequest flood wait» in `[путь владельца]` on the hub — THAT log, not `~/.claude/scripts/_logs/telegram_mcp_sse.log`, which is the HP17 launcher's). Before restarting anything: read `_sse.err.log`; if flood-wait lines are fresh, the daemon is healthy — switch to STEP 0 (`find_chat.py` → id-based calls) and the same request answers instantly. Upstream: chigwell/telegram-mcp issue #180.

## 5) Common recipes
- **Find a chat by title:** `search_dialogs("name fragment")`.
- **Read a chat's recent activity:** `get_history(chat_id, limit=N)` (newest-first).
- **Pull today's content dictations:** `get_history(-[id], limit~40)` → keep Anton's text msgs + "Personal Audio Summary" transcripts dated today.
- **Send Anton a draft:** `send_message([id], "<text>")` (plain; no parse_mode for Cyrillic bodies).
- **Get a login/OTP code (self-serve):** `get_history(777000, account=<acct>, limit=1)` → take `Login code: NNNNN`. Don't ask Anton — and the 2FA password is self-serve too, from `~/.claude/secrets/telegram_2fa.env` (see §above). Full login flow = `/tg-connect`.
...
The connector at `[путь владельца]` is a git checkout of upstream `github.com/chigwell/telegram-mcp`. We have LOCAL changes that a naive reinstall/`git pull` could lose:
...
- **Re-apply after a reinstall:** `cd [путь владельца] && git apply $IMPORTS_ROOT/content-factory/telegram-mcp-local-tools.patch`, then restart the connector.
