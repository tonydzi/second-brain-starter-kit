...
1. **In-session tool check FIRST (zero-risk)** — if a server's tools are present in the session, call ONE cheap read tool to confirm auth + liveness:
   - Telegram → `mcp__telegram__get_me` (expect Tony / @[рабочий аккаунт]).
