---
name: notpeople-wave
description: "- One command to run the next NotPeople investor-outreach wave end-to-end, the verified way. Use when the operator says “продолжаем рассылку NotPeople“, “возьми следующих N инвесторов“, “следующая партия“, “/notpeople-wave“, “новая волна NotPeople“"
version: 1.0.0
---
...
**What this is.** NotPeople = a $600K pre-seed SAFE raise pitched to ranked crypto/VC investors over Telegram from **@[рабочий аккаунт]**. [коллега] is the live lead operator; this (follower) machine runs the sends. Project memory: `notpeople-outreach-waves`. Pipeline lives in `[путь владельца]` (on the hub: `$IMPORTS_ROOT/notpeople/` — drive-agnostic scripts).
...
2. Sending blind from the ranked queue → duplicate sends, a wrong name (draft "[человек]" → actually [человек]), and dead handles.
...
- Confirm which machine you're on. **Follower ([коллега], vault on D:, receive-only):** local CSV/dashboard edits may be reverted by Syncthing → you MUST also relay the pitched-record to the hub (step 6). **Hub (`[машина флота]`):** you are the source of truth, no relay needed.

### 1. Select the next N truly-new investors
- Default N = 10 (operator may say a number). The operator may name leads to **skip** (e.g. "[человек] пропусти") → drop them, pull the next-ranked to refill N.
- Run the selector: `python "[путь владельца]"` (reads the scored pool, excludes anyone already in `NotPeople-Pitched.csv`).
...
- `resolve_username(@handle, account="[рабочий аккаунт]")` → **GEN-ERR-659 = dead/changed handle → drop it**, refill from next-ranked.
- `get_history(<chat>, account="[рабочий аккаунт]", limit=…)` → read the actual thread.
  ...
  - Name mismatch between the draft and the real Telegram display name → **fix the name** (the "[человек]"→"[человек]" bug).
...
- Pitches come from `build_pitch_drafts.py` (50 personalized, ranked) — `python "[путь владельца]"` writes `pitch_queue.json` + a drafts note. Top-N = the batch. Personalize per lead; **@[рабочий аккаунт] = Anton's personal voice → Opus**, no copy-paste blast, pace the sends ([[telegram-lead-outreach]] guardrails).
...
- `send_message(<chat>, <text>, account="[рабочий аккаунт]")` per lead.

### 4. Verify delivery per-lead (don't assume)
- After the batch, `get_history(<chat>, account="[рабочий аккаунт]", limit=1)` for each → confirm the outbound msg landed (capture msg_id + timestamp). Report "N/N delivered" with the ids. This is the "ты уверен, что отправка ушла?" check — do it without being asked.
...
- **Follower machine:** relay the pitched-record to the hub so the real dedup lands there too: `python "%USERPROFILE%\.claude\scripts\machine_bus.py" send [машина флота] "NotPeople wave <date>: appended N rows to Pitched.csv (<handles>); prune dead handles <…> from pool."`
...
- **HUB never messages leads** — leads are worked by the live operator ([коллега]). On the hub, this skill prepares/records but does not send. ([[leads-live-operator-check-thread]].)
- No em/en dashes anywhere (memory `no-long-dashes`).

## Files & values (reuse, don't recreate)
- Send account: `@[рабочий аккаунт]` ([рабочий аккаунт], Tony SF / Bay Area).
...
- GEN-ERR-659 = dead/changed handle. machine_bus relay target = `[машина флота]`.
