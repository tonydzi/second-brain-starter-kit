---
name: bus
description: "Cross-machine talk between Anton's computers over a shared TELEGRAM GROUP (primary rail) — every machine posts/reads in one group via the Telegram MCP, so it “just works“ even when Syncthing is down, AND humans… Trigger on “/bus“, “/tg“, “шина“, “напиши в шину“, “глянь шину“, “что в шине“, “скажи другим компам“, “передай на <машину>“, “post to the bus“, “check the bus“, “tell the other machines“"
version: 1.0.0
---

# /bus — компы Антона болтают в общей Telegram-группе

**Зачем.** Раньше связь Claude↔Claude между машинами шла ТОЛЬКО через синканную папку `[шина]` (Syncthing). Если синк лёг / машина офлайн / файл ещё едет — связь молчит. **Telegram-группа — облако: каждая машина достаёт сама, "просто работает", и люди ВИДЯТ, как компы переписываются.** Поэтому:

- **Telegram-группа = ОСНОВНОЙ канал** (этот скилл).
- **`[шина]` (Syncthing) = ЗАПАСНОЙ** — только для гигантов (>4096 символов: лимит Telegram) или когда Telegram недоступен. Запасной = скилл `/inbox`, движок `machine_bus.py`.

Транспорт делает **Telegram-MCP** (его видит каждая машина), бухгалтерию (кто я, что уже прочитал, формат конверта, фильтр «что мне новое») — детерминированный `tg_bus.py` (0 токенов). Аккаунт, от которого постят/читают ВСЕ машины, — в `~/.claude/tg_bus.json` (по умолчанию `[рабочий аккаунт]`, обезличенный служебный голос).
...
1. Собери конверт детерминированно (правильный тег + проверка длины):
   ...
   ```
   `<TARGET>` = имя машины-получателя (напр. `[машина флота]`, `[машина флота]`) или `ALL` (всем). Скрипт напечатает готовую строку вида `[BUS] [машина флота] -> ALL: текст`. Если он предупредил, что длина > 4096 — **НЕ шли в Telegram**, уходи в запасной канал (`machine_bus.py send ...`, см. §4).
...
1. **Syncthing `[шина]`** — НОРМА (файловая шина, ~10с, для структуры/файлов/больших). Скилл `/inbox`.
...
## Связанное
- `/inbox` — запасной канал (Syncthing `[шина]`).
