...
| `Set-Cookie` names | Framework identification without any version string: `PHPSESSID` (PHP), `JSESSIONID` ([человек]), `ASP.NET_SessionId` (.NET), `laravel_session`, `wp-settings-*` (WordPress), `csrftoken` plus `sessionid` (Django) |
...
| Default pages | "Welcome to nginx!", the Apache distribution default page, an IIS splash, or a framework setup [человек] mean an unconfigured or forgotten host. Setup wizards mean uninitialized, which is a serious finding |
