...
| `k8s`, `ingress`, `rancher`, `openshift`, `consul`, `etcd`, `[человек]` | Container platform. Implies a cluster, and cluster ingress naming conventions to enumerate | Convention discovery: if `ingress-a` exists, so do siblings |
