...
description: "- Recover deleted, edited or historical web content using the Wayback Machine and its CDX API, archive.today, Common Crawl… Use when a page is deleted, changed or 404s, checking what a site used to say, finding old team or staff pages, prior pricing, removed posts, pre-redaction wording or old contact details, enumerating every archived URL for a domain, or preserving evidence before it disappears"
---
...
```bash
curl -s 'https://web.archive.org/web/[id]id_/http://example.com/pricing' > pricing-2019.html
```
...
- **Capture time is not publication time.** A page first captured in [человек] may
