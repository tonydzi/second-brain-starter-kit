...
CONTEXT:
- Канонический реестр (этот хаб): `[путь владельца]` (есть и копия в проекте ноута — читать хабовую).
