---
name: watch-channel
description: "Put a NIGHTLY alpha watcher on ANY Telegram channel/chat in one command - the “make /mine-channel a routine“ button. Trigger on “/watch-channel <@channel|id>“, “следи за каналом <X>“, “поставь ночной watcher на <канал>“, “мониторь канал <X>“, “watch this channel nightly“, “ночной майнер на <канал>“"
version: 1.0.0
---
...
**1. Резолв канала → id + slug.** Юзернейм (`[аккаунт]`) или числовой id. Не знаешь id - `/chat <имя>` или `mcp__telegram__search_dialogs`. Выбери короткий латинский **slug** (он же папка `_imports\alpha\<slug>`).
...
**4. Поставить/убедиться в ночной задаче - НА ХАБЕ** ([[hub-master-machine]], [[desktop-max-laptop-min]]; ночное окно [[routines-run-at-night]]). Все авто-телеграм-джобы консолидированы на always-on хабе. Если `WatchChannelsNightly` ещё нет - завести её там (через `[шина]` задачу хабу, [[machine-bus-telegram-rail]]):
...
- `_imports` НЕ синкается → застейджить движок в `[шина]\_transit\_imports-engines\watchers\` и попросить хаб скопировать.
