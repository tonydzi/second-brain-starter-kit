...
- Send `Authorization: Bearer $SCREENPIPE_LOCAL_API_KEY`, `X-Screenpipe-Client: api`,
  `X-Screenpipe-Agent: cursor`; save with `curl --fail-with-body -o response.json`.
...
| `tags` | No | Comma-separated; returns items carrying ALL of them (`person:ada,project:[человек]`). Exact match. |
...
- `/speakers/update` `{"id":29,"name":"[человек]"}` — rename/metadata
- `/speakers/reassign` `{"audio_chunk_id":456,"new_speaker_name":"[человек]","propagate_similar":true}` — returns `new_speaker_id`, `transcriptions_updated`, `old_assignments` (for undo)
...
**"That was actually [человек], not [человек]":** find the audio result's `chunk_id` → `POST /speakers/reassign` with `audio_chunk_id` + `new_speaker_name`; `propagate_similar:true` (default) also fixes similar chunks.
...
# Actions persist to the notification [человек], so the user can approve later even
