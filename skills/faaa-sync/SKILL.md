---
name: faaa-sync
description: "FAAA follow-up звонки: On-demand pull of FRESH FAAA follow-up call notes from Anton's Telegram group “CALLS 889 MAIN FA FAAAA follow up“ (chat -[id]) into the Obsidian Platinum-CRM. Trigger on “/faaa-sync“, “подтяни свежие FAAA“, “забери свежие звонки“, “обнови FAAA“, “sync FAAA calls“, or whenever Anton wants the latest call follow-ups in the vault without waiting for Monday's auto-run."
version: 1.0.0
---
...
CONTEXT:
- Source: Telegram supergroup "CALLS 889 MAIN FA FAAAA follow up MAIN (ТОЛЬКО итоги звонков и НИЧЕГО БОЛЬШЕ)", chat_id = -[id] (read via the Telegram MCP; logged-in account @[рабочий аккаунт]).
...
4. Pull history: call the Telegram MCP get_new_messages_since for chat_id -[id] with min_id = WM, limit 200 (or get_history limit 200 then filter id > WM). If the oldest returned id is still > WM, fetch again with a larger window to cover the whole gap.
...
6. Write the new messages (id > WM) to $IMPORTS_ROOT/faaa/faaa-new-msgs.json as:
   {"chat_id":-[id],"watermark":WM,"results":[{"id":<int>,"sender":<from>,"date":<iso8601>,"text":<FULL verbatim text>,"media":<media type if any>}]}
...
- Team handles (@[рабочий аккаунт], @[рабочий аккаунт], [аккаунт]*, [аккаунт]*, [аккаунт]*) are NOT leads — the scripts already exclude them.
