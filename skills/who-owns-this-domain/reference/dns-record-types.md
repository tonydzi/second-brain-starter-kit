...
|---|---|---|---|
| `A` / `AAAA` | `dig +short d A` | Hosting IP. Feed to ASN lookup and `find-exposed-servers`. | CDN/proxy IPs hide the origin. Multiple As can be round-[человек], GeoDNS, or a shared load balancer. IPv6 is often less carefully firewalled than IPv4 — and sometimes points at the real origin when the A record points at a CDN. |
