---
name: relink
description: "- “Перелинковка важного“ — on-command BIDIRECTIONAL integration of a new IMPORTANT node (concept, mental model, framework, theory, project, principle, research cluster) into the WHOLE vault graph, not just creating a note. Trigger on “/relink“, “перелинкуй важное“, “перелинковка важного“, “интегрируй концепт <X>“, “вплети это в граф“, “/relink --deep“"
version: 1.0.0
---
...
- **Caps — по КЛАССУ заметки, не глобально** (DR 01.09): лист (обычная evergreen) = 1 navigation ingress + 0–3 semantic, hard cap 5–7, ≤3 per section; **P2/P3 на листьях НЕ писать вовсе**; лист↔лист почти не линковать — длинные рёбра только в MOC/identity/decision (hub-prior); MOCs exempt.
...
- `relink_last_run` = **cooldown 90 дней** (DR 01.09): заметку со свежим штампом Mode B/C не пересуживают; штамп БЕЗ этого чтения не писать (мёртвый frontmatter на 194k = шум синка).
...
## Mode B — `--deep` (узкая очередь curated-сирот, НЕ месячный проход по всем)
⭐ ПЕРЕДЕЛАН 2026-09-01 по DR26-09-01-HUB-01-0938 (консенсус chatgpt+grok): «39k сирот» — вредный KPI, ~95% умышленные (импорты/session-md/drafts). Mode B работает ТОЛЬКО с классом curated/navigation; ручная глаз-фильтрация папок УДАЛЕНА из ритуала — её заменяет политика.
1. `python "$IMPORTS_ROOT/orphan-scan/orphan_scan.py"` → refresh `orphans.csv` ([[vault-orphan-baseline]]).
2. **`python "$IMPORTS_ROOT/orphan-scan/curated_queue.py" --top 30`** → классы по `orphan_policy.json` (curated · navigation · generated_pending · raw_archive · derived_machine · ephemeral) + ранжированная очередь по value_score (+бонусы: MOC-сирота, decision/reglament, свежесть). Замер 01.09: 38 931 → **570 реального долга** (68×). Правка скоупа = правка `orphan_policy.json`, не кода и не глаз.
3. Работай очередь сверху через Mode A steps 2–7. `generated_pending` — НЕ твой долг: это долг ПИСАТЕЛЯ (link-at-birth, п.6); raw_archive/derived_machine/ephemeral не судим вовсе.
4. Dashboard (`build_dashboard.py`) — по надобности; dupes → skill `dedup`; new MOC для стабильного кластера 5+ можно тут.
5. Report what got woven in + what was deliberately left (по классам, с цифрами среза).
6. **Link-at-birth контракт (anton 01.09.2026)** — sibling rule, чинит КОРЕНЬ сирот: авто-писатель рождает заметку в курируемых слоях СО связями (≥1 typed outgoing + дорога из хаба), иначе работа не done. Mode B находит нарушителя → чинить ПИСАТЕЛЯ, не заметку. Дверь = `~/.claude/scripts/link_at_birth_lint.py` (суточный, housekeeping_daily; ledger `link_at_birth_violations.jsonl`). Канон: волт `02-Protocols/reglament-link-at-birth-avtopisateli-rozhdayut-zametki-so-svyazyami` + [[link-at-birth-contract]]. Proven first run 2026-09-01 (Mode B): 17 typed links / 10 notes / 0 broken; ~95% сирот умышленные (импорты/session-md/drafts) — фильтруй их ДО ручного разбора.

## Mode C — relink a whole THEME / topic cluster ("перелинкуй всё про X") — ГЛАВНЫЙ САНИТАР
⭐ DR 01.09: Mode C — основной режим оздоровления графа (подтверждённая «три ноги»-эвристика = community detection без Leiden), Mode B — лишь узкая очередь top-N curated. Between A and B: Anton names a TOPIC, not one note. Run **Mode A steps 2–7 with the THEME's entities as the query** (RAG + namesearch + grep on theme terms → judge top-K). Then, before proposing links:
...
- Proven runs:
  ...
  - 2026-06-20 (father / Олег [человек]): all three legs missing in the integration — created **person-card** `person-[человек]-[человек]` + **`MOC-Family-Life`** (closed long-standing TODO in `MOC-index`) + identity link in `_Self-Bible-MOC` (father = "identity-brick of the twin"). 5 notes / ~12 typed links / 0 broken.

## Метрики и следующие ступени (DR26-09-01-HUB-01-0938; строить по гейту §3.5, не сейчас)
- **Меряем retrieval lift, не «минус N сирот»**: связать с A/B пилотом recall [[always-on-memory-pilot]] третьим плечом (C = vector+graph+НОВЫЕ рёбра relink); delta Recall@K ≈ 0 на semantic `see_also` → перестать их писать (резать типы записи, не наращивать сканер).
- **Edge grammar → TSV/SQLite индекс триплетов** (строгая строка в «## Связано» + производный пересобираемый индекс; НЕ graph DB) — даст запрос «все evidence_for решения X». Ценность 4.5/сложность 2 — первый кандидат следующей итерации.
- **Счётчики orphan births по писателю/батчу** — видимость корня по производителю (ledger уже пишет `link_at_birth_lint.py`; сводку по writer добавить туда же).
- ДР-вердикт по архитектуре: retrieval→judgment→writing = already-SOTA для markdown+wikilinks; KG-embeddings (TransE и др.), полный GraphRAG, GNN-центральность — НЕ переносить (open-world PKM ≠ закрытый KG).
