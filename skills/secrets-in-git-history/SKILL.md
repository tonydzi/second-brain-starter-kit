...
description: "- Mine GitHub, GitLab and git history for identities, infrastructure and leaked credentials using commit author emails, GitHub code search, the commit… Use when investigating a developer or organisation on GitHub, finding leaked API keys, AWS keys or tokens in code, enumerating org members and their personal repos, or recovering secrets deleted from HEAD but still present in history or forks"
---
...
newer form `[id]+username@users.noreply.github.com`. That second form leaks
...
```bash
curl -s https://api.github.com/user/[id] | jq '.login, .created_at'
```
...
repositories. Stars and follows are the underused ones: nobody [человек] them for
...
One author commits as `[id]+dhaverford@users.noreply.github.com`. Resolving
