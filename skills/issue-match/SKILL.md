---
name: issue-match
description: Замер очереди ЧУЖОГО репозитория + поиск ЖИВОЙ двери (открытого issue под наш артефакт) ДО того, как пушить туда PR. Триггеры "/issue-match", "/issue", "замерь очередь", "куда пушить PR", "есть ли живая дверь", "стоит ли пушить в <репо>", "проверь репо перед PR", "issue-first", "куда контрибьютить", "measure the queue", "find a live issue". Отвечает на вопрос «стучаться сюда или мимо»: сколько открытых PR, медиана возраста, доля без единого комментария, как выглядят реальные мержи, сколько НАШИХ PR там лежит холодными, и есть ли открытое issue, на которое наш артефакт отвечает. Вердикт 🟢 живая дверь / 🟡 двери нет / 🔴 глухая очередь + черновик issue-first-входа. READ-ONLY: ничего не пушит, не комментит, не открывает issue — решение и отправка всегда за человеком. Движок `~/.claude/scripts/issue_match.py`, 0 LLM, 0 токенов на замер, единственная зависимость — `gh`. Канон: память [[cold-pr-into-silent-queue]] (правило Антона 25.07: замер очереди — ПЕРВЫЙ шаг, до написания кода), [[ship-github-no-plus-wait]], [[github-publish-autonomous]], Библия `reglament-zametnost-v-top-10-llm-issue-matching`. Сосед — `/release-slice` (публикация НАШИХ кусков в НАШ GitHub, другая лейна) и сторож `pr_watch.py` (следит за УЖЕ открытыми PR постфактум).
license: MIT
---
